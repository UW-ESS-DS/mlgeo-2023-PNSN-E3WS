...
# Retrieved from: https://github.com/alexburky/rflexa/blob/master/transfer/python/transfer.py
# Retrieved by: Nathan T. Stevens
# Retrieved on: 28. November 2023
# License: Copyright (c) 2020 Alexander Burky 
# ---------------------------------------------------------
...
def transfer(data, delta, freq_limits, units, file, file_type):
    ...
    if units == "displacement":
        ...
        print(
            "Error: Currently accepted output units are displacement, velocity, or acceleration"
        )
    ...
    iresp[1:] = np.vectorize(complex)(
        np.multiply(h[1:].real, denr[1:]), np.multiply(-h[1:].imag, denr[1:])
    )
    ...
    data_fft[-1] = complex(
        np.sqrt(
            data_fft[-1].real * data_fft[-1].real
            + data_fft[-1].imag * data_fft[-1].imag
        ),
        0,
    )
