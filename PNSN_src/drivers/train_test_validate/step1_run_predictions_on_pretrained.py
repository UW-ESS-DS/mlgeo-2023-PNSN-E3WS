...
fv_data = os.path.join(ROOT,
                       'PNSN_data',
                       'bulk_event_magnitude_phase_nwf_FV.csv'
                       )
...
for _k in models.keys(): # for Benz / Jake change to: "for _k in ['MAG']:"
    ...
    for _i in tqdm(range(len(df))):#len(df))):
        ...
        # If the feature vector contains nan or inf, return nan for ensemble and member predictions
            ...
            pred = np.nan
            segment_pred_line = [np.nan]*len(_model.base_models[0])
...
df_out.to_csv(os.path.join(OUT_ROOT,

                           'event_mag_phase_FV_pretrained_ENSEMBLE_predictions.csv'),
              header=True,
              index=False
              )
...
for _k in segment_pred_dict.keys():
    ...
    idf.to_csv(
        os.path.join(
            OUT_ROOT,
            f'event_mag_phase_FV_pretrained_individual_{_k}_predictions.csv'),
        header=True,
        index=True
    )